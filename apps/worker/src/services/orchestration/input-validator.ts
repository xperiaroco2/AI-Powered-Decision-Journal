...
export class InputValidator {
  ...
  validate(input: unknown): ValidationResult {
    ...
    errors.push(...situationQuality.errors, ...decisionQuality.errors, ...reasoningQuality.errors);
    warnings.push(
    ...
    if (data.situation.toLowerCase().includes(data.chosenDecision.toLowerCase())) {
      warnings.push("Decision text appears to be copy-pasted from situation - consider being more specific");
    }

    // 4. Determine overall quality
    const quality = this.determineQuality(situationQuality, decisionQuality, reasoningQuality);
  ...
  private validateSituation(situation: string): {
    ...
    if (/(.)\1{4,}/.test(situation)) {
      errors.push("Situation contains repeated characters - appears to be gibberish");
    }

    // Check for minimum word count
    const wordCount = situation.split(/\s+/).length;
    if (wordCount < 10) {
      errors.push("Situation is too brief - provide at least 10 words of context");
    } else if (wordCount < 20) {
      warnings.push("Situation is brief - more context would improve analysis quality");
    }

    // Check for question marks (indicates uncertainty about what to provide)
    const questionCount = (situation.match(/\?/g) || []).length;
    if (questionCount > 3) {
      warnings.push("Situation contains many questions - focus on describing the context");
    }

    // Check for URLs (spam indicator)
    if (/https?:\/\//.test(situation)) {
      warnings.push("Situation contains URLs - ensure this is relevant context");
    }
  ...
  private validateDecision(decision: string): {
    ...
    if (/(.)\1{4,}/.test(decision)) {
      errors.push("Decision contains repeated characters - appears to be gibberish");
    }
    ...
    const actionVerbs = /\b(decided|chose|will|going to|plan to|accepted|rejected|bought|sold|quit|started|stopped)\b/i;
    if (!actionVerbs.test(decision)) {
      warnings.push("Decision doesn't clearly state what action was taken - consider rephrasing");
    }
  ...
  private validateReasoning(reasoning: string): {
    ...
    if (/(.)\1{4,}/.test(reasoning)) {
      errors.push("Reasoning contains repeated characters - appears to be gibberish");
    }

    // Check for minimum word count
    const wordCount = reasoning.split(/\s+/).length;
    if (wordCount < 5) {
      warnings.push("Reasoning is very brief - more detail would improve analysis");
    }
