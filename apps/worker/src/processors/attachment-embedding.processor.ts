...
export async function processAttachmentEmbedding(
  ...
  return context.with(parentCtx, () =>
    tracer.startActiveSpan(
      ...
      async (span) => {
        log.info(
          { jobId: job.id, attachmentId, filename },
        ...
        try {
          ...
          if (filename) {
            try {
              ...
              log.info(
                { jobId: job.id, charsExtracted: textContent.length },
                'File parsed successfully',
              );

              if (parseResult.metadata) {
                log.info(
                  { jobId: job.id, metadata: parseResult.metadata },
                  'File metadata',
                );
              }
          ...
          log.info(
            { jobId: job.id, stats: getChunkingStats(chunkingResult) },
            'Chunking complete',
          );

          if (chunkingResult.wasTruncated) {
            log.warn(
              { jobId: job.id, maxChunks: DEFAULT_CHUNK_CONFIG.maxChunks },
          ...
          if (deletedChunks.count > 0) {
            log.info(
              { jobId: job.id, deletedCount: deletedChunks.count },
              'Deleted existing chunks (re-run)',
            );
          }

          // 6. Create chunks in database
          log.info(
            { jobId: job.id, chunkCount: chunkingResult.chunks.length },
            'Creating chunks',
          );
          ...
          log.info(
            { jobId: job.id, createdCount: createdChunks.length },
            'Created chunks in database',
          );

          // 7. Generate embeddings for each chunk
          log.info(
            { jobId: job.id, chunkCount: createdChunks.length },
              ...
              try {
                for (const [index, dbChunk] of createdChunks.entries()) {
                  try {
                    ...
                    const embeddingVector =
                      await embeddingProvider.generateEmbedding(
                        dbChunk.content,
                      );
                ...
                if (failureCount > 0) {
                  batchSpan.setStatus({
                    ...
                    message: `${failureCount} chunk(s) failed`,
                  });
                }
                batchSpan.end();
              }
          ...
          log.info(
            { jobId: job.id, successCount, failureCount },
            'Embedding complete',
          );
          ...
          if (failureCount === 0) {
            ...
            log.warn(
              { jobId: job.id, failureCount },
              'Attachment processing completed with failures',
            );
